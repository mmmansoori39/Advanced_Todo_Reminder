...
const deleteTask = async (id)=>{
  ...
  return (
    <>  
        ...
        <div className='mt-20'>
        <p className='text-center text-zinc-600 sm:mt-24 mt-28 font-semibold' >HI YOU HAVE COMPLETED {data.length} TASKS </p>
      {
          <div >
            <div className=" w-300 sm:w-[700px] card">
              ...
              
              {
                data.map( (tasks)=>{
                  return(
                    <div key={tasks._id}  className= ' line-through bg-white p-2' style={{borderBottom:'1px solid lightgray', display:'flex', justifyContent:'space-between', alignItems:'center'}} >
                      <div className="">
              ...
              {/* <div className="text-center bg-white ">
                <button onClick={ deleteCompletedTask } className="ClearAllBtn rounded px-4 py-2 sm:text-sm text-xs  " style={{ backgroundColor:'teal',margin:'1rem'}}>Delete All</button>
              </div> */}
            </div>
          </div>
